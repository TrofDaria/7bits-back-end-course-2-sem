...
public class DatabaseTasksRepositoryTest {
    ...
    public void testGetTasks() {
        ...
        List<Task> mockListTasks = mock(List.class);
        String order = "desc";
        when(mockJdbcOperations.query(anyString(), any(PreparedStatementSetter.class), any(RowMapper.class))).thenReturn(mockListTasks);

        List<Task> expectedList = databaseTasksRepository.getTasks(anyString(), order, anyInt(), anyInt());
        verify(mockJdbcOperations, times(1)).query(
                eq("SELECT id, text, status, created_at, updated_at FROM " +
                        "task WHERE status = ? " +
    ...
    @Test
    public void testGetTasksAmount() {
        JdbcOperations mockJdbcOperations = mock(JdbcOperations.class);
        DatabaseTasksRepository databaseTasksRepository = new DatabaseTasksRepository(mockJdbcOperations);

        int actual = 10;
        String status = "done";
        when(mockJdbcOperations.queryForObject(anyString(), any(RowMapper.class), anyString())).thenReturn(actual);

        int expected = databaseTasksRepository.getTasksAmount(status);
